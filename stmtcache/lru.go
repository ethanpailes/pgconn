...
func (c *LRU) Get(ctx context.Context, sql string) (*pgconn.StatementDescription, error) {
	...
	if c.conn.TxStatus() == 'I' && len(c.stmtsToClear) > 0 {
...
func (c *LRU) StatementErrored(ctx context.Context, sql string, err error) error {
	...
	// NOTE: we could probably get away with testing c.conn.TxStatus() != 'E' here, but we will never
	//       really have a situation where we are getting told that a statement just errored while
	//       the current transaction is still healthy unless the client is using this cache in a
	//       really weird way.
	if c.conn.TxStatus() == 'I' {
		return c.clearStmt(ctx, sql)
	}
